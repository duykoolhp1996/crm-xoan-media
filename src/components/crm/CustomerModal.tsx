...
import { X, Sparkles, User, School, Calendar, DollarSign, Tag, MapPin } from 'lucide-react';
...
export const CustomerModal: React.FC<CustomerModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] overflow-y-auto p-4 sm:p-6 flex items-center justify-center animate-in fade-in duration-200">
      ...
      <div className="relative w-full max-w-3xl bg-neutral-900/90 backdrop-blur-3xl rounded-3xl border border-white/20 shadow-[0_32px_80px_rgba(0,0,0,0.8),inset_0_1px_1px_rgba(255,255,255,0.18)] overflow-hidden my-8 text-white">
        {/* Modal Header */}
        <div className="px-6 py-4 bg-white/[0.03] border-b border-white/[0.08] text-white flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-sm font-bold tracking-tight">Thêm Mới Lead / Khách Hàng Kỷ Yếu</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6 max-h-[80vh] overflow-y-auto text-xs custom-scrollbar">
          ...
          <div className="space-y-3">
            <h3 className="font-bold text-white uppercase tracking-wider flex items-center gap-1.5 border-b border-white/[0.08] pb-2 text-[11px]">
              <User className="w-4 h-4 text-orange-400" /> 1. Thông Tin Người Đại Diện
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                <label className="font-medium text-white/70">Họ & Tên khách *</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full mt-1 px-3 py-2 glass-input rounded-xl"
                />
              </div>
              <div>
              ...
              <div>
                <label className="font-medium text-white/70">Vai trò trong lớp</label>
                <select
                  value={formData.representativeRole}
                  onChange={e => setFormData({ ...formData, representativeRole: e.target.value })}
                  className="w-full mt-1 px-3 py-2 glass-input rounded-xl cursor-pointer [&>option]:bg-neutral-900 [&>option]:text-white"
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                <label className="font-medium text-white/70">Link Facebook</label>
                <input
                  type="text"
                  placeholder="fb.com/..."
              ...
              <div>
                <label className="font-medium text-white/70">Số Zalo</label>
                <input
                  type="text"
              ...
              <div>
                <label className="font-medium text-white/70">Email</label>
                <input
                  type="email"
                  placeholder="email@..."
          ...
          <div className="space-y-3">
            <h3 className="font-bold text-white uppercase tracking-wider flex items-center gap-1.5 border-b border-white/[0.08] pb-2 text-[11px]">
              <School className="w-4 h-4 text-sky-400" /> 2. Thông Tin Trường & Lớp Học
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
              <div className="sm:col-span-2">
                <label className="font-medium text-white/70">Trường học</label>
                <input
                  type="text"
              ...
              <div>
                <label className="font-medium text-white/70">Tên Lớp *</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="font-medium text-white/70">Sỉ số (Học sinh)</label>
                <input
                  type="number"
                  min={10}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              <div>
                <label className="font-medium text-white/70">Khối lớp</label>
                <select
              ...
              <div>
                <label className="font-medium text-white/70">Niên khóa</label>
                <input
                  type="text"
              ...
              <div>
                <label className="font-medium text-white/70 flex items-center gap-1">
                ...
                <select
                  ...
                  className="w-full mt-1 px-3 py-2 glass-input rounded-xl cursor-pointer font-medium text-orange-300 [&>option]:bg-neutral-900 [&>option]:text-white"
                >
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full mt-1 px-3 py-2 glass-input rounded-xl cursor-pointer font-medium text-sky-300 [&>option]:bg-neutral-900 [&>option]:text-white"
                >
          ...
          <div className="space-y-3">
            <h3 className="font-bold text-white uppercase tracking-wider flex items-center gap-1.5 border-b border-white/[0.08] pb-2 text-[11px]">
              <Calendar className="w-4 h-4 text-purple-400" /> 3. Nhu Cầu Kỷ Yếu & Concept
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                <label className="font-medium text-white/70">Gói Kỷ Yếu Quan Tâm</label>
                <select
              ...
              <div>
                <label className="font-medium text-white/70">Concept yêu thích</label>
                <input
                  type="text"
              ...
              <div>
                <label className="font-medium text-white/70">Ngân sách dự kiến (đ)</label>
                <input
                  type="number"
                  step={500000}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="font-medium text-white/70">Ngày dự kiến chụp</label>
                <input
                  type="date"
                  value={formData.expectedShootDate}
                  onChange={e => setFormData({ ...formData, expectedShootDate: e.target.value })}
                  className="w-full mt-1 px-3 py-2 glass-input rounded-xl cursor-pointer"
                />
              </div>
              <div>
                <label className="font-medium text-white/70">Địa điểm dự kiến</label>
                <input
                  type="text"
          ...
          {/* Section 4: Nguồn Marketing Attribution */}
          <div className="space-y-3">
            <h3 className="font-bold text-white uppercase tracking-wider flex items-center gap-1.5 border-b border-white/[0.08] pb-2 text-[11px]">
              <Tag className="w-4 h-4 text-emerald-400" /> 4. Nguồn Tiếp Cận (Marketing Attribution)
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                <label className="font-medium text-white/70">Kênh nguồn *</label>
                <select
                  value={formData.source}
                  onChange={e => setFormData({ ...formData, source: e.target.value as LeadSource })}
                  className="w-full mt-1 px-3 py-2 glass-input rounded-xl font-semibold cursor-pointer [&>option]:bg-neutral-900 [&>option]:text-white"
                >
              ...
              <div>
                <label className="font-medium text-white/70">Tên chiến dịch (Campaign)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="font-medium text-white/70">Trạng thái ban đầu</label>
                <select
          ...
          <div className="pt-4 border-t border-white/[0.08] flex items-center justify-end gap-3">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 glass-btn-secondary rounded-xl font-semibold transition-colors"
            >
              Hủy
            </button>
            <button
              type="submit"
              className="px-6 py-2 glass-btn-primary rounded-xl font-semibold transition-all"
            >
