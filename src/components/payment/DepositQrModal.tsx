...
export const DepositQrModal: React.FC<DepositQrModalProps> = ({
  ...
  const transferSyntax = isFinalPayment
    ? `THANH TOAN KYYEU ${cleanClassName} ${cleanPhone}`
    : `COC KYYEU ${cleanClassName} ${cleanPhone}`;

  // Link ảnh VietQR chuẩn Napas 247
  const vietQrUrl = `https://img.vietqr.io/image/${bankConfig.bankId}-${bankConfig.accountNumber}-compact2.png?amount=${paymentAmount}&addInfo=${encodeURIComponent(transferSyntax)}&accountName=${encodeURIComponent(bankConfig.accountName)}`;

  const depositPercent = totalBudget > 0 ? Math.round((paymentAmount / totalBudget) * 100) : 30;
...
📦 Gói dịch vụ: ${customer.servicePackageName || 'Kỷ Yếu Concept & Dạ tiệc'}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
💰 BẢNG QUYẾT TOÁN CHI PHÍ HỢP ĐỒNG:
• Tổng kinh phí hợp đồng: ${totalBudget.toLocaleString('vi-VN')} đ
• Số tiền cọc đã thanh toán: ${paidSoFar.toLocaleString('vi-VN')} đ
👉 SỐ TIỀN CÒN LẠI CẦN THANH TOÁN HẾT: ${paymentAmount.toLocaleString('vi-VN')} đ
...
Bạn mở App Ngân Hàng quét mã QR đính kèm để tự động điền đúng STK, Số tiền và Nội dung thanh toán!
...
👤 Người đại diện: ${customer.name} (${customer.representativeRole || 'Lớp trưởng'}) - ${customer.phone}
💰 Tổng kinh phí hợp đồng: ${totalBudget.toLocaleString('vi-VN')} đ
👉 SỐ TIỀN CỌC ĐỢT 1: ${paymentAmount.toLocaleString('vi-VN')} đ (${depositPercent}% giữ lịch chụp)
    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-3 sm:p-5 bg-black/80 backdrop-blur-sm overflow-hidden animate-in fade-in duration-150">
      <div className="relative w-full max-w-4xl bg-white rounded-3xl shadow-2xl border border-black/[0.08] overflow-hidden max-h-[92vh] h-[92vh] flex flex-col">
        ...
        <div className={`p-4 sm:px-6 text-white flex items-center justify-between gap-3 shrink-0 ${
          isFinalPayment ? 'bg-gradient-to-r from-teal-950 via-neutral-900 to-emerald-950' : 'bg-neutral-900'
        }`}>
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-2xl flex items-center justify-center font-black shrink-0 shadow-sm ${
              isFinalPayment ? 'bg-teal-400 text-teal-950' : 'bg-[#B8F23D] text-neutral-950'
            }`}>
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h2 className="text-sm sm:text-base font-bold text-white">
                  {isFinalPayment ? 'Tất Toán & Tạo QR Thanh Toán Hết' : 'Tạo Cọc Giữ Lịch & Mã QR Chuyển Khoản'}
                </h2>
                <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full font-mono ${
                  isFinalPayment ? 'bg-teal-400/20 text-teal-300' : 'bg-[#B8F23D]/20 text-[#B8F23D]'
                }`}>
                  {customer.className}
                </span>
              ...
              <p className="text-[11px] text-neutral-300 mt-0.5">
                {isFinalPayment
                  ? 'Sinh mã VietQR quyết toán toàn bộ số tiền còn lại sau khi đã bàn giao sản phẩm kỷ yếu'
        ...
        {isSuccess && (
          <div className="p-4 bg-emerald-500 text-white text-center font-bold text-sm flex items-center justify-center gap-2 animate-in slide-in-from-top">
            ...
            <span>
              {isFinalPayment
                ? 'Đã xác nhận tất toán thành công! Khách hàng đã được chuyển sang "Hoàn thành".'
                : 'Đã xác nhận nhận cọc thành công! Khách hàng đã được chuyển sang "Đã đặt cọc".'}
            </span>
          </div>
        )}
        ...
        <div className="p-4 sm:p-6 overflow-y-auto flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 bg-[#fafafa] custom-scrollbar overscroll-contain">
          ...
          <div className="lg:col-span-7 space-y-4">
            ...
            {/* Thông tin lớp học & Quyết toán chi phí */}
            <div className="p-4 bg-white rounded-2xl border border-black/[0.06] shadow-xs">
              ...
              {/* Bảng phân tích chi phí Hợp Đồng */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-3 text-xs bg-neutral-50 p-3 rounded-xl border border-black/[0.04]">
                <div>
                  <span className="text-[10px] text-neutral-400 font-medium block">1. Tổng kinh phí (Bill):</span>
                  <p className="font-extrabold text-neutral-900 text-sm mt-0.5">
                    {totalBudget > 0 ? `${totalBudget.toLocaleString('vi-VN')} đ` : 'Chưa nhập'}
                  </p>
                  <p className="text-[10px] text-neutral-500 truncate">{customer.servicePackageName || 'Gói Kỷ Yếu'}</p>
                </div>

                <div>
                  <span className="text-[10px] text-neutral-400 font-medium block">2. Đã đặt cọc:</span>
                  <p className="font-bold text-emerald-700 text-sm mt-0.5">
                    {paidSoFar > 0 ? `${paidSoFar.toLocaleString('vi-VN')} đ` : '0 đ (Chưa cọc)'}
                  </p>
                  <p className="text-[10px] text-neutral-500">
                    {totalBudget > 0 ? `${Math.round((paidSoFar / totalBudget) * 100)}% tổng bill` : '0%'}
                  </p>
                </div>

                <div className={`${isFinalPayment ? 'bg-teal-50 border border-teal-200 p-2 rounded-lg' : ''}`}>
                  <span className="text-[10px] font-bold text-neutral-700 block">
                    {isFinalPayment ? '👉 CẦN TẤT TOÁN:' : '3. Còn lại dự kiến:'}
                  </span>
                  <p className={`font-black text-sm mt-0.5 ${isFinalPayment ? 'text-teal-700 text-base' : 'text-neutral-900'}`}>
                    {remainingAmount.toLocaleString('vi-VN')} đ
                  </p>
                  <p className="text-[10px] text-neutral-500">
                    {isFinalPayment ? '(Tổng bill - Cọc)' : 'Thu đợt cuối'}
                  </p>
                </div>
              </div>

              <div className="mt-3 text-xs flex items-center justify-between text-neutral-600">
                <span className="text-[11px]">Đại diện: <strong>{customer.name}</strong> ({customer.representativeRole || 'Lớp trưởng'})</span>
            ...
            <div className="p-4 bg-white rounded-2xl border border-black/[0.06] shadow-xs space-y-3">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full text-xl sm:text-2xl font-black text-neutral-900 px-4 py-2.5 rounded-xl border border-black/[0.1] focus:outline-none focus:border-[#84cc16] bg-neutral-50/50 font-mono"
                />
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {isFinalPayment ? (
                  <>
                    ...
                    >
                      Tất toán 100% ({remainingAmount.toLocaleString('vi-VN')}đ)
                    </button>
                    {remainingAmount > 0 && (
                      <button
                        type="button"
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                          paymentAmount === Math.round(remainingAmount * 0.5)
                            ? 'bg-teal-700 text-white shadow-xs'
                            : 'bg-neutral-100 hover:bg-neutral-200 text-neutral-700'
                        }`}
                      >
                        50% còn lại ({Math.round(remainingAmount * 0.5).toLocaleString('vi-VN')}đ)
                      </button>
                    )}
                  </>
                ) : (
                  [
                    ...
                    <button
                      key={idx}
                      type="button"
                      onClick={() => setPaymentAmount(chip.value)}
                      className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                        paymentAmount === chip.value
            ...
            <div className="p-4 bg-white rounded-2xl border border-black/[0.06] shadow-xs space-y-3">
              <h3 className="text-xs font-bold text-neutral-900 uppercase tracking-wide flex items-center gap-1.5">
                ...
                Thông Tin Tài Khoản Nhận Thanh Toán (Studio)
              </h3>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={isFinalPayment ? "Ví dụ: Đã kiểm tra đủ album & quà tặng..." : "Ví dụ: Giữ slot ekip chụp ngày 28/11/2026..."}
          ...
          <div className="lg:col-span-5 flex flex-col justify-between space-y-4">
            ...
            <div className={`p-5 bg-white rounded-3xl border-2 shadow-md flex flex-col items-center text-center relative overflow-hidden ${
              isFinalPayment ? 'border-teal-400' : 'border-[#B8F23D]'
            }`}>
            ...
            {/* NÚT XÁC NHẬN: CHỐT CỌC HOẶC TẤT TOÁN HOÀN THÀNH */}
            <div className={`p-4 rounded-2xl border shadow-xs space-y-2.5 ${
              isFinalPayment
                ? 'bg-gradient-to-br from-teal-50 via-emerald-50 to-white border-teal-200'
                : 'bg-gradient-to-br from-emerald-50 via-lime-50 to-white border-emerald-200/80'
            }`}>
              <div className={`flex items-center gap-2 ${isFinalPayment ? 'text-teal-900' : 'text-emerald-900'}`}>
                <Sparkles className={`w-4 h-4 ${isFinalPayment ? 'text-teal-600' : 'text-emerald-600'}`} />
                <span className="text-xs font-bold">
                  {isFinalPayment ? 'Xác Nhận Tất Toán Hợp Đồng' : 'Xác Nhận Thu Cọc'}
                </span>
              </div>
              <p className="text-[11px] text-neutral-600 leading-relaxed">
                {isFinalPayment
                  ? 'Khi ban cán sự lớp đã chuyển khoản tất toán đủ số tiền còn lại, bấm nút dưới đây để hoàn tất hợp đồng và chuyển sang "Hoàn thành".'
                  : 'Khi ban cán sự lớp đã chuyển khoản cọc thành công, bấm nút dưới đây để hệ thống tự động ghi nhận và chuyển sang giai đoạn "Đã đặt cọc".'}
              </p>
